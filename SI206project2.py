...
## Create regular expressions, use Regex 101 website
## In order for a link to be valid there must be two characters after the dot
...
def grab_headlines(): #Refer to Newyork times example
...
def get_umsi_data():
    ...
    for names in soup.find_all(class_= "field-name-title"):
        for firstname_lastname in names.find_all('h2'):
            numofnames.append(firstname_lastname.text)


    for title in soup.find_all(class_= "field field-name-field-person-titles field-type-text field-label-hidden"):
        for title_name in title.find_all('h2'):
            numoftitle.append(title_name.text)

    for info in directory:
        information = {'numofnames': title[info]} 
        directory.append(information)

        # print(information)
            
    # for n in numofnames:
    #     directory[n].append(numofnames)

    # for t in numoftitle:
    #     directory[t].append(numoftitle)
